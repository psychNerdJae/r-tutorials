...
candy <- read_csv(paste0(filepath, "/candyhierarchy2017.csv"),
                  local = locale(encoding = "latin1")) %>%
  pivot_longer(cols=starts_with("Q6 |"), names_to="item", values_to="rating") %>%
  clean_names() %>%
  select(sub=internal_id, goingOut=q1_going_out, gender=q2_gender, age=q3_age, item, rating)
